...
app.get('/image/:annotationId/:base64Url/image.png', async (req: Request, res: Response) => {
    ...
    try {
        ...
        console.log(`[DEBUG] Annotation found with screenshot:`, annotation.screenshot.length, 'bytes');
        let imageData: Buffer;
        try {
            // Strip data:image/png;base64, prefix if present
            const base64String = annotation.screenshot.startsWith('data:image/png;base64,')
                ? annotation.screenshot.replace('data:image/png;base64,', '')
                : annotation.screenshot;
            imageData = Buffer.from(base64String, 'base64');
            console.log(`[DEBUG] Decoded Base64 screenshot to buffer: ${imageData.length} bytes`);
        } catch (error) {
            console.error(`[DEBUG] Failed to decode Base64 screenshot:`, error);
            return res.status(400).send('Invalid screenshot data');
        }

        res.set('Content-Type', 'image/png');
        console.log(`[DEBUG] Sending PNG image response for /image`);
        res.send(imageData);
    } catch (error) {
...
// Updated /viewannotation route with image URL
